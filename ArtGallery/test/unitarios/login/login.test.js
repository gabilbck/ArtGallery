...
describe("POST /login (unitário)", () => {
    ...
    it("deve mostrar erro se email ou senha estiverem vazios", async () => {
        const res = await request(app).post("/").send({ email: "", senha: "" });
    ...
    it("deve autenticar com sucesso se usuário for válido", async () => {
        ...
        const res = await request(app).post("/").send({ email: "maria@email.com", senha: "123" });
    ...
    it("deve mostrar erro se usuário for inválido", async () => {
        ...
        const res = await request(app).post("/").send({ email: "naoexiste@email.com", senha: "errado" });
    ...
    it("deve mostrar erro de servidor se buscarUsuario falhar", async () => {
        ...
        const res = await request(app).post("/").send({ email: "falha@email.com", senha: "123" });
